...
int Minpath(int mode, int Orig, int* PredLink, double* CostTo) {
    ...
    now = g_map_external_node_id_2_node_seq_no[Orig]; // mapping from external zone id of Orig (which is defined in
                                                      // demand.csv_ to the corresponding node id (== zone_id) and then
                                                      // to the node internal number
    ...
    while ((now != INVALID) && (now != WAS_IN_QUEUE)) {
        if (now >= FirstThruNode ||
            now == internal_node_id_for_origin_zone) // this is the key implementation for FirstThruNode on connector
        {
...
vector<int> Processor_origin_zones[50];
                    ...
                    for (int Dest = 1; Dest <= no_zones; Dest++) {
                        ...
                        if (MDODflow[m][Orig][Dest] > 0.000001) {
                            ...
                            if (CostTo[Orig][internal_node_id_for_destination_zone] <= BIGM - 1) // feasible cost
                            {
                                ...
                                system_least_travel_time_org_zone[Orig] +=
                                    MDRouteCost[m][Orig][Dest] * MDODflow[m][Orig][Dest] * g_mode_type_vector[m].pce;
                            }
                            else {
                                int debug_flag = 1;
                            }
...
vector<vector<vector<vector<vector<int>>>>> linkIndices;

void AddLinkSequence(int m, int Orig, int Dest, int route_id, const vector<int>& linkIDs) {
    if (linkIndices.empty()) {
        return;
    }

    // Ensure we are within bounds before adding the link sequence
    if (Orig > 0 && Orig < linkIndices[m].size() && Dest > 0 && Dest < linkIndices[m][Orig].size() && route_id >= 0 &&
        route_id < linkIndices[m][Orig][Dest].size()) {
        linkIndices[m][Orig][Dest][route_id] = linkIDs; // Add link sequence to the 5D vector
    }
    else {
        cerr << "Error: Invalid indices for adding link sequence.\n";
    }
}
...
void All_or_Nothing_Assign(int Assignment_iteration_no, double*** ODflow, int*** MinPathPredLink, double* Volume) {
    ...
    if (ProcessorVolume == nullptr) {
        cerr << "Error: Memory allocation for ProcessorVolume failed.\n";
    ...
    if (ProcessorModeVolume == nullptr) {
        cerr << "Error: Memory allocation for ProcessorModeVolume failed.\n";
        // Handle the error, e.g., exit the program, clean up resources, etc.
        exit(EXIT_FAILURE);
    }
...
#pragma omp parallel for
    ...
    if (Assignment_iteration_no == 0) {
        ...
        for (int Orig = 1; Orig <= no_zones; Orig++) {
            ...
            if (g_map_external_node_id_2_node_seq_no.find(Orig) != g_map_external_node_id_2_node_seq_no.end() &&
                TotalOFlow[Orig] < 0.00001) // only work on positive zone flow
            {
                printf("%d,", Orig);
            }
        }
        ...
        for (int Orig = 1; Orig <= no_zones; Orig++) {
            ...
            if (g_map_external_node_id_2_node_seq_no.find(Orig) != g_map_external_node_id_2_node_seq_no.end() &&
                zone_outbound_link_size[Orig] == 0) // there is no outbound link from the origin
            {
                printf("%d,", Orig);
            }
        }
...
#pragma omp parallel for
    for (int p = 0; p < g_number_of_processors; p++) {
        ...
        for (int i = 0; i < Processor_origin_zones[p].size(); i++) {
            ...
            int CurrentNode;
            double RouteFlow;
            vector<int> currentLinkSequence; // Temporary vector to store link indices
...
#pragma omp parallel for
    ...
    auto minutes = duration_cast<std::chrono::minutes>(duration % std::chrono::hours(1));
    auto seconds = duration_cast<std::chrono::seconds>(duration % std::chrono::minutes(1));
    auto milliseconds = duration_cast<std::chrono::milliseconds>(duration % std::chrono::seconds(1));

    printf("All or nothing assignment: %ld hours %ld minutes %lld seconds %lld ms\n", hours.count(), minutes.count(),
...
//---------------------------------------------------------------------------
vector<double> computeTheta(const vector<double>& lambda) {
...
void performODME(const vector<double>& theta, double* MainVolume, struct link_record* Link) {
    // Open log file (this will overwrite any existing file with the same name)
    ofstream logFile("ODME_log.txt");
    if (!logFile.is_open()) {
        cerr << "Error: Could not open ODME_log.txt for writing.\n";
        return;
    }
    ...
    auto computeObjectiveForMDOD = [&](double*** mdod) -> double {
        ...
        vector<double> linkFlows(number_of_links + 1, 0.0);
        for (int m = 1; m <= numModes; ++m) {
            for (int Orig = 1; Orig <= numZones; ++Orig) {
                for (int Dest = 1; Dest <= numZones; ++Dest) {
                    ...
                    int nRoutes = min(theta.size(), linkIndices[m][Orig][Dest].size());
    ...
    for (int iter = 0; iter < maxIter; ++iter) {
        ...
        // --- Log simple deviation measures ---
        {
            ...
            for (size_t i = 0; i < linkFlows.size(); ++i) {
                if (Link[i].Obs_volume > 1) {
                    totalLinkDev += fabs(linkFlows[i] - Link[i].Obs_volume);
                    linkCount++;
                }
            }
            ...
            logFile << "[Deviation Log] OD avg deviation = " << avgODDev << ", Link avg deviation = " << avgLinkDev
                    << ", VMT deviation = " << vmtDev << "\n";
        }

        // Allocate a 3D structure for gradients (assumed same dimensions as MDODflow)
        vector<vector<vector<double>>> gradMDOD(
            numModes + 1, vector<vector<double>>(numZones + 1, vector<double>(numZones + 1, 0.0)));
        ...
        for (int m = 1; m <= numModes; ++m) {
            for (int Orig = 1; Orig <= numZones; ++Orig) {
                for (int Dest = 1; Dest <= numZones; ++Dest) {
                    ...
                    int routeCount = 0;
                    unordered_map<int, int> linkUsage;
                    ...
                    // (Depending on your formulation, you might want to recompute the sensitivities using the updated
                    // link flows.)
        ...
        if (sqrt(sumGradSq) < tol) {
            logFile << "Convergence reached after " << iter << " iterations.\n";
            cout << "Convergence reached after " << iter << " iterations.\n";
            break;
        }
        ...
        while (true) {
            ...
            logFile << "[Line Search] alpha = " << alpha << " | F_candidate = " << F_candidate
                    << " | Threshold = " << (F_current - armijo_c * alpha * sumGradSq) << "\n";
            // Check the Armijo condition:
            if (F_candidate <= F_current - armijo_c * alpha * sumGradSq) {
                logFile << "[Line Search] Armijo condition met: F_candidate (" << F_candidate << ") <= F_current ("
                        << F_current << ") - " << armijo_c << " * alpha (" << alpha << ") * sumGradSq (" << sumGradSq
                        << ")\n";
                break;
            }
            else {
        ...
        logFile << "Iteration " << iter << ": Step size chosen = " << alpha << ", Objective: " << F_current << " -> "
                << computeObjectiveForMDOD(candidateMDOD) << "\n";
    ...
    vector<double> linkFlows(number_of_links + 1, 0.0);
    vector<vector<double>> mode_linkFlows(numModes + 1, vector<double>(number_of_links + 1, 0.0));
    for (int m = 1; m <= numModes; ++m) {
        for (int Orig = 1; Orig <= numZones; ++Orig) {
            for (int Dest = 1; Dest <= numZones; ++Dest) {
                if (Orig == Dest) {
...
void performODME_2(vector<double> theta, double* MainVolume, struct link_record* Link) {
    // Open log file (this will overwrite any existing file with the same name)
    ofstream logFile("ODME_log.txt");
    if (!logFile.is_open()) {
        cerr << "Error: Could not open ODME_log.txt for writing.\n";
    ...
    for (int iter = 0; iter < maxIter; ++iter) {
        ...
        //	if (iter == 0)
        {
            ...
            logFile << "  OD Demand deviations: max = " << maxDemandDev << ", average = " << avgDemandDev << "\n";
            ...
            for (size_t i = 0; i < linkFlows.size(); ++i) {
                if (Link[i].Obs_volume > 1) {
                    double dev = fabs(linkFlows[i] - Link[i].Obs_volume);
                    maxLinkDev = max(maxLinkDev, dev);
                    sumLinkDev += dev;
                    total_obj_deviation += dev;
            ...
            logFile << "  Link flow deviations: max = " << maxLinkDev << ", average = " << avgLinkDev << "\n";
        ...
        for (int m = 1; m <= numModes; ++m) {
            for (int Orig = 1; Orig <= numZones; ++Orig) {
                for (int Dest = 1; Dest <= numZones; ++Dest) {
                    ...
                    int routeCount = 0;
                    unordered_map<int, int> linkUsage; // key: link_id, value: count of routes using this link
                    for (int route_id = 0; route_id < (int)linkIndices[m][Orig][Dest].size(); ++route_id) {
                        if (linkIndices[m][Orig][Dest][route_id].empty()) {
                            continue;
                        }
                    ...
                    maxGrad = max(maxGrad, fabs(total_grad));
                    ...
                    if (fabs(total_grad) > 1e-3) {
                        logFile << "    [Mode " << m << " OD(" << Orig << "," << Dest << ")] "
                                << "od_flow=" << od_flow << ", grad_link=" << grad_link << ", grad_vmt=" << grad_vmt
                                << ", grad_od=" << grad_od << ", total_grad=" << total_grad
                                << ", step size=" << step_size
...
struct RouteData {
    ...
    string nodeIDsStr; // Node IDs (string)
    string linkIDsStr; // Link IDs (string)
    double totalDistance;
    double totalFreeFlowTravelTime;
    double totalTravelTime;
    double accumulatedTheta = 0; // Sum of theta for all routes with this key
    string routeKey;        // Unique key (e.g., based on node and link sums)
};

void OutputRouteDetails(const string& filename, const vector<double>& theta) {
    ...
    outputFile << "mode,route_id,o_zone_id,d_zone_id,unique_route_id,prob,node_ids,link_ids,distance_mile,total_"
                  "distance_km,total_free_flow_travel_"
                  "time,total_travel_time,route_key,volume,\n";
    ...
    for (int m = 1; m < linkIndices.size(); ++m) {
        for (int Orig = 1; Orig < linkIndices[m].size(); ++Orig) {
            for (int Dest = 1; Dest < linkIndices[m][Orig].size(); ++Dest) {
                unordered_map<string, RouteData> uniqueRoutes;
                    ...
                    if (!linkIndices[m][Orig][Dest][route_id].empty()) {
                        ...
                        for (int i = linkIndices[m][Orig][Dest][route_id].size() - 1; i >= 0; --i) {
                            ...
                            if (i == 0) {
                                ...
                                nodeIDsStr += to_string(toNodeID);
                                nodeSum += toNodeID;
                            }
                        }

                        // Create a unique key for the route.
                        string routeKey = to_string(nodeSum) + "_" + to_string(linkSum);
                ...
                for (const auto& pair : uniqueRoutes) {
                    ...
                    string cleanedLinkIDsStr = rd.linkIDsStr;
                    ...
                    outputFile << g_mode_type_vector[m].mode_type.c_str() << "," << rd.firstRouteID
                               ...
                               << Orig << "," << Dest << "," << rd.unique_route_id << "," << rd.accumulatedTheta << ","
                               << rd.nodeIDsStr << "," << cleanedLinkIDsStr << "," << rd.totalDistance << ","
                               << rd.totalDistance * 1.609 << "," << rd.totalFreeFlowTravelTime << ","
                               << rd.totalTravelTime << "," << rd.routeKey << "," << route_volume << "\n";
                }
            }
        }
    }
...
string convert_minutes_to_hhmmss(double minutes) {
...
vector<double> calculate_departure_time(double T0, double T1, double T3, double beta, int route_volume) {
...
void OutputVehicleDetails(const string& filename, const vector<double>& theta) {
    ...
    // Write the CSV header in lowercase
    outputFile << "agent_id,departure_time,departure_time_hhmmss,mode,route_id,o_zone_id,d_zone_id,unique_route_id,"
                  "node_ids,link_ids,total_distance_"
                  "mile,total_distance_km,total_free_flow_travel_time,total_travel_time,route_key,route_volume,\n";
            ...
            for (int Dest = 1; Dest < linkIndices[m][Orig].size(); ++Dest) {
                ...
                for (const auto& pair : uniqueRoutes) {
                    ...
                    if (!linkIndices[m][Orig][Dest][rd.firstRouteID].empty()) {
                        ...
                        vector<double> linkTravelTimes; // Store travel times for each link
                        ...
                        if (integer_volume > 0) {
                            ...
                            for (int v = 0; v < integer_volume; ++v) {
                                ...
                                if (integer_volume > 10) {
                                    ...
                                    departure_time =
                                        demand_period_starting_hours * 60 + agent_id % total_time_window_min;
                                    time_increment_in_min = 1;
                                }

                                vector<double> nodeArrivalTimes; // Store arrival times for each node
                                ...
                                if (!timeSequenceStr.empty()) {
                                    timeSequenceStr.pop_back();
                                }

                                // departure_time += std::rand() * time_increment_in_min;

                                string departure_time_hhmmss = convert_minutes_to_hhmmss(departure_time);

                                // Write the data for this OD pair and route to the CSV file
                                outputFile << agent_id << "," << departure_time << "," << departure_time_hhmmss << ","
                                           << g_mode_type_vector[m].mode_type.c_str() << "," << rd.firstRouteID << ","
                                           << Orig << "," << Dest << "," << unique_route_id << "," << nodeIDsStr << ","
                                           << linkIDsStr << "," << totalDistance << "," << totalDistance * 1.609 << ","
                                           << totalFreeFlowTravelTime << "," << totalTravelTime << ","
                                           << rd.firstRouteID << "," << route_volume << "\n";
...
void OutputODPerformance(const string& filename) {
    std::ofstream outputFile(filename); // Open the file for writing

    // Write the CSV header in lowercase
    outputFile << "mode,o_zone_id,d_zone_id,o_x_coord,o_y_coord,d_x_coord,d_y_coord,total_distance_mile,total_distance_"
                  "km,total_free_flow_travel_"
                  "time,total_congestion_travel_time,volume,\n";
    ...
    for (int m = 1; m < linkIndices.size(); ++m) {
        for (int Orig = 1; Orig < linkIndices[m].size(); ++Orig) {
            for (int Dest = 1; Dest < linkIndices[m][Orig].size(); ++Dest) {
                unordered_map<string, bool> uniqueRoutes;
                int unique_route_id = 1;
                for (int route_id = 0; route_id < linkIndices[m][Orig][Dest].size(); ++route_id) {
                    if (!linkIndices[m][Orig][Dest][route_id].empty()) {
                        double totalDistance = 0.0;
                        ...
                        if (uniqueRoutes.find(routeKey) == uniqueRoutes.end()) {
                            ...
                            outputFile << g_mode_type_vector[m].mode_type.c_str() << "," << Orig << "," << Dest << ","
                                       ...
                                       << totalFreeFlowTravelTime << "," << totalTravelTime << "," << volume << "\n";
    ...
    cout << "OD performance summary: avg distance = " << grand_totalDistance / grand_total_count << " miles, "
              << ", avg free-flow travel time = " << grand_totalFreeFlowTravelTime / grand_total_count << " min, "
              << ", avg total travel time = " << grand_totalTravelTime / grand_total_count << " min"
              << ", avg travel time index = " << grand_totalTravelTime / grand_totalFreeFlowTravelTime << " "
              << std::endl;
    // Close the file after writing
    outputFile.close();
    cout << "Output written to " << filename << std::endl;
...
void createSettingsFile(const string& fileName) {
    ofstream file(fileName);
    if (!file.is_open()) {
        cerr << "Could not create the file: " << fileName << '\n';
        return;
    }

    // Writing the headers of the CSV
    file << "metric_system,number_of_iterations,number_of_processors,demand_period_starting_hours,demand_period_ending_"
            "hours,base_demand_mode,route_"
            "output,vehicle_output,log_file,odme_mode,odme_vmt\n";

    // Writing the sample data (from your provided file)
    file << "0,10,8,7,18,0,0,0,0,0,0\n";

    file.close();
    cout << "sample_settings.csv file created successfully!" << std::endl;
}
...
void createModeTypeFile(const string& fileName) {
    ofstream file(fileName);
    if (!file.is_open()) {
        cerr << "Could not create the file: " << fileName << '\n';
        return;
    ...
    if (parser_mode_type.OpenCSVFile("mode_type.csv", true)) {
                                              ...
                                              // field headers.
        {
            ...
            if (number_of_modes < MAX_MODE_TYPES) {
                ...
                parser_mode_type.GetValueByFieldName("dedicated_shortest_path",
                                                     g_mode_type_vector[number_of_modes].dedicated_shortest_path);
...
void InitializeLinkIndices(int num_modes, int no_zones, int max_routes) {
    ...
    linkIndices = vector<vector<vector<vector<vector<int>>>>>(
        num_modes + 1, vector<vector<vector<vector<int>>>>(
                           no_zones + 1, vector<vector<vector<int>>>(
                                             no_zones + 1, vector<vector<int>>(max_routes + 1))));
    ...
    printf("Memory creation time for 5D link path matrix: %ld hours %ld minutes %lld seconds %lld ms\n",
...
int AssignmentAPI() {
    ...
    printf(
        "# of nodes= %d, largest zone id (# of zones) = %d, First Through Node (Seq No) = %d, number of links = %d\n",
    ...
    SDVolume = (double*)Alloc_1D(number_of_links,
                                 sizeof(double)); /* Compute search direction and sub-volume in the same place. */
    SubVolume = (double*)Alloc_1D(number_of_links,
                                  sizeof(double)); /* Compute search direction and sub-volume in the same place. */
    ...
    fprintf(link_performance_file,
            ...
            "capacity,D,doc,fftt,travel_time,VDF_alpha,VDF_beta,VDF_plf,speed_mph,speed_kmph,VMT,VHT,PMT,PHT,VHT_QVDF,"
            "PHT_QVDF,geometry,");

    fprintf(logfile, "iteration_no,link_id,from_node_id,to_node_id,volume,ref_volume,obs_volume,capacity,doc,fftt,"
                     "travel_time,delay,");
    ...
    fprintf(link_performance_file,
            "P,t0,t2,t3,vt2_mph,vt2_kmph,mu,Q_gamma,free_speed_mph,cutoff_speed_mph,free_speed_kmph,cutoff_speed_kmph,"
            "congestion_ref_speed_mph,avg_queue_speed_mph,avg_QVDF_period_speed_mph,congestion_ref_speed_kmph,avg_"
            "queue_speed_"
            "kmph,avg_QVDF_period_speed_kmph,avg_QVDF_period_travel_time,Severe_Congestion_P,");
    ...
    All_or_Nothing_Assign(iteration_no, MDDiffODflow, MDMinPathPredLink,
                          MainVolume); // here we use MDDiffODflow as our OD search direction of D^c - D^b,
    ...
    if (g_tap_log_file == 1) {
        for (int k = 1; k <= number_of_links; k++) {
            fprintf(logfile, "%d,%d,%d,%d,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,", iteration_no, k,
                    ...
                    Link[k].Obs_volume, Link[k].Link_Capacity, MainVolume[k] / fmax(0.01, Link[k].Link_Capacity),
    ...
    duration<double, std::milli> duration_FindMinCostRoutes;
    duration<double, std::milli> duration_All_or_Nothing_Assign;
    duration<double, std::milli> duration_LinksSDLineSearch;
    auto start0 = high_resolution_clock::now(); // Start timing
    ...
    for (iteration_no = 1; iteration_no < TotalAssignIterations; iteration_no++) {
        ...
        auto end1 = high_resolution_clock::now();                       // End timing
        auto start2 = high_resolution_clock::now();                     // Start timing
        ...
        auto end2 = high_resolution_clock::now(); // End timing
        ...
        fprintf(summary_log_file,
                "iter No = %d, Lambda = %f, g_System_VMT = %f, sys. TT =  %lf, least TT =  %lf, gap = %f %% \n",
                iteration_no, Lambda, g_System_VMT, system_wide_travel_time, system_least_travel_time, gap);

        if (g_tap_log_file == 1) {
            for (int k = 1; k <= number_of_links; k++) {
                fprintf(logfile, "%d,%d,%d,%d,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,", iteration_no, k,
                        Link[k].external_from_node_id, Link[k].external_to_node_id, MainVolume[k], Link[k].Ref_volume,
                        Link[k].Link_Capacity, MainVolume[k] / fmax(0.01, Link[k].Link_Capacity),
                        Link[k].FreeTravelTime, Link[k].Travel_time, Link[k].Travel_time - Link[k].FreeTravelTime);
    ...
    printf("CPU running time: %ld hours %ld minutes %lld seconds %lld ms\n", hours.count(), minutes.count(),
           seconds.count(), milliseconds.count());

    // Log the result with hours, minutes, seconds, and milliseconds
    fprintf(summary_log_file, "CPU running time: %ld hours %ld minutes %lld seconds %lld ms\n", hours.count(),
    ...
    for (int k = 1; k <= number_of_links; k++) {
        ...
        fprintf(link_performance_file,
                "%d,%d,%d,%d,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,",
                iteration_no, Link[k].link_id, Link[k].external_from_node_id, Link[k].external_to_node_id,
                MainVolume[k], Link[k].Ref_volume, Link[k].Base_volume, Link[k].Obs_volume, Link[k].Link_Capacity,
                IncomingDemand, DOC, Link[k].FreeTravelTime, Link[k].Travel_time, Link[k].VDF_Alpha, Link[k].VDF_Beta,
                Link[k].VDF_plf, Link[k].length / fmax(Link[k].Travel_time / 60.0, 0.001),
    ...
    if (parser_link.OpenCSVFile("link.csv", true)) {
                                         ...
                                         // field headers.
        {
            ...
            if (lanes <= 0 || capacity < 0.0001 || free_speed < 0.0001) {
                cerr << "Error: Invalid link attributes detected. "
                          << "lanes = " << lanes << ", capacity = " << capacity << ", free_speed = " << free_speed
                          << ". Skipping this link.\n";
                continue;
            }
            ...
            if (g_base_demand_mode == 1) {
                ...
                for (int m = 1; m <= number_of_modes; m++) {
                    string field_name = "base_vol_" + string(g_mode_type_vector[m].mode_type);
            ...
            if (Link[k].allowed_uses.size() > 0 && Link[k].allowed_uses != "all") {
                for (int m = 1; m <= number_of_modes; m++) {
                    if (Link[k].allowed_uses.find(g_mode_type_vector[m].mode_type) !=
                        string::npos) // otherwise, only an agent type is listed in this "allowed_uses", then this
...
void StatusMessage(const char* group, const char* format, ...) { double new_time; }
...
int Read_ODtable(double*** ODtable, double*** DiffODtable, double*** target_ODtable, int no_zones) {
    ...
    for (int m = 1; m <= number_of_modes; m++) {
                ...
                if (number_of_modes == 1) {
                    for (int o = 1; o <= no_zones; o++) {
                        for (int d = 1; d <= no_zones; d++) {
                            ...
                            if (g_map_external_node_id_2_node_seq_no.find(o) !=
                                    g_map_external_node_id_2_node_seq_no.end() &&
                                g_map_external_node_id_2_node_seq_no.find(d) !=
                                    g_map_external_node_id_2_node_seq_no.end()) {

                                ODtable[m][o][d] = 1.0;
                                DiffODtable[m][o][d] =
                                    1.0; // by default in case there is no baseline demand being specified
                            }
                        }
                    }
                }
        ...
        while ((result = fscanf(file, "%d,%d,%lf", &o_zone_id, &d_zone_id, &volume)) != EOF) {
            ...
            if (result == 3) // we have read all the 3 values correctly
            {
                ...
                DiffODtable[m][o_zone_id][d_zone_id] =
                    volume; // by default in case there is no baseline demand being specified
                total_volume += volume;
                line_count++;
            }
            else {
    ...
    if (g_base_demand_mode == 1 && baselinkvolume_loaded_flag == 1) {
        ...
        for (int m = 1; m <= number_of_modes; m++) {
            FILE* file;
            string original_filename = g_mode_type_vector[m].demand_file;
            ...
            if (file == NULL) {
                // by default, we can skip this requirement, but if we load baseline link volume we should have base OD
            ...
            while ((result = fscanf(file, "%d,%d,%lf", &o_zone_id, &d_zone_id, &volume)) != EOF) {
                if (result == 3) // we have read all the 3 values correctly
                {
                    ...
                    DiffODtable[m][o_zone_id][d_zone_id] =
                        ODtable[m][o_zone_id][d_zone_id] - volume; // diff OD demand  = current demand - base demand
    ...
    if (g_ODME_mode == 1) {
        for (int m = 1; m <= number_of_modes; m++) {
            FILE* file;
            const string& original_filename = g_mode_type_vector[m].demand_file;
            string modified_filename;
...
double Link_QueueVDF(int k, double Volume, double& IncomingDemand, double& DOC, double& P, double& t0, double& t2,
    ...
    Q_mu = min(Link[k].Lane_Capacity, IncomingDemand / max(0.01, P));
    ...
    for (int t_in_min = demand_period_starting_hours * 60; t_in_min <= demand_period_ending_hours * 60;
         t_in_min += 5) // 5 min interval
    {
...
double Link_Travel_Time_Integral(int k, double* Volume) {
    double IncomingDemand = Volume[k] / fmax(0.001, demand_period_ending_hours - demand_period_starting_hours) /
                            fmax(0.0001, Link[k].VDF_plf);
    double integral = 0;
...
double OF_LinksDirectionalDerivative(double* MainVolume, double* SDVolume, double Lambda) {
    //
    // purpose:
    //    this function calculates the directional derivative of the objective function with respect to the step size
    //    lambda.in optimization, the directional derivative indicates how the objective function changes as you move in
    ...
    // The directional derivative represents how the total system cost changes as you adjust the flow along the search
...
int Read_ODflow(double* TotalODflow, int* number_of_modes, int* no_zones) {
    ...
    if (total_infeasible_outbound_zones) {
        printf("Error: %d zones have no outbound link with positive demand %f, = %f percentage of total demand\n",
               total_infeasible_outbound_zones, total_infeasible_outbound_zone_demand,
               total_infeasible_outbound_zone_demand * 100 / fmax(0.01, total_zone_demand));
    }
...
class SimulationLogger {
  ...
  public:
    ...
    void log(LogLevel level, const string& event, int agent_id, int link_id, int node_id, const string& details) {
        ...
        if (level == LOG_ERROR) {
            error_file << timestamp << " [ERROR] " << event << " Agent:" << agent_id << " Link:" << link_id
                       << " Node:" << node_id << " - " << details << '\n';
        }

        // Detailed debugging info
        if (level <= LOG_DEBUG) {
            debug_file << timestamp << "," << getLevelString(level) << "," << event << "," << agent_id << "," << link_id
                       << "," << node_id << "," << details << '\n';
        }
    ...
    void logAgentMovement(int t, int agent_id, int from_link, int to_link, const string& status) {
        if (LOG_TRACE <= current_level) {
            debug_file << t << ",TRACE,MOVE," << agent_id << "," << from_link << "->" << to_link << "," << status
                       << '\n';
        }
    }
    ...
    void logQueueStatus(int t, int link_id, int entrance_size, int exit_size) {
        if (LOG_DEBUG <= current_level) {
            debug_file << t << ",DEBUG,QUEUE," << link_id << "," << entrance_size << "," << exit_size << '\n';
        }
    }

    void logSimulationStatus(int t, int loaded, int transfers, int completed) {
        if (LOG_INFO <= current_level) {
            info_file << t << ",INFO,STATUS," << loaded << "," << transfers << "," << completed << '\n';
        }
    }
...
class CAgent_Simu {
  public:
    ...
    CAgent_Simu()
        : agent_uid(-1), agent_seq_no(-1), distannce(0), travel_time(0), departure_time_in_min(0),
          departure_time_in_simu_interval(0), current_link_seq_no(0), loaded(false),
...
int g_ParserIntSequence(string str, vector<int>& vect) {
    stringstream ss(str);
    int i;
    while (ss >> i) {
        vect.push_back(i);
...
int read_vehicle_file(vector<shared_ptr<CAgent_Simu>>& agents) {
    ...
    if (!parser_vehicle.OpenCSVFile("vehicle.csv", true)) {
        cerr << "Error: Cannot open vehicle.csv\n";
        return 0;
    }
...
class TrafficSimulator {
  private:
    ...
    void initializeMemory() { state.initialize(numLinks, demand_period_starting_hours, demand_period_ending_hours); }
  ...
  public:
    TrafficSimulator(int links, int intervals) : numLinks(links), numIntervals(intervals) { initializeMemory(); }

    ~TrafficSimulator() { state.cleanup(); }
};
...
class LinkQueue {
  public:
    ...
    LinkQueue() : capacity_per_time_step(0), free_flow_time(0), number_of_lanes(1), length(0), spatial_capacity(0) {}
...
private:
    ...
    string formatTimesVector(const vector<double>& times, int l_demand_period_starting_hours,
        ...
        for (size_t i = 0; i < times.size(); ++i) {
            ...
            if (times[i] >= 0) {
                ss << convertTimeToHHMMSS(l_demand_period_starting_hours * 60.0 +
                                          times[i] * number_of_seconds_per_interval / 60.0);
            }
            else {
                ss << "NA";
            }
...
public:
    AgentLogger() {
        ...
        // Write header
        agent_file
            << "agent_id,departure_time,departure_time_hhmmss,loaded_status,o_zone_id,d_zone_id,distance,travel_time,"
    ...
    void logAgent(const shared_ptr<CAgent_Simu>& agent, int l_demand_period_starting_hours) {
        ...
        double travel_time = (agent->link_departure_times[agent->current_link_seq_no] - agent->link_arrival_times[0]) *
                             number_of_seconds_per_interval / 60.0;

        agent_file << agent->agent_uid << "," << agent->departure_time_in_min << ","
                   << convertTimeToHHMMSS(agent->departure_time_in_min) << "," << (agent->loaded ? "1" : "0") << ","
                   << agent->o_zone_id << "," << agent->d_zone_id << "," << agent->distannce << ","
                   << agent->current_link_seq_no << "," << vectorToString(agent->path_link_sequence) << ","
...
#pragma omp parallel for
    ...
    void logSimulationStatus(int t) {
        ...
        if (t % 600 == 0) { // Log every minute
            ...
            debug_file << t << ",status," << total_loaded_agents << "," << total_link_transfers << ","
                       << total_node_transfers << ",minute_summary\n";
...
#pragma omp atomic
                        total_completed_trips++;
                        debug_file << t << ",complete," << agent->agent_uid << "," << link_id << ",0,trip_completed\n";
                        continue;
                    }
...
#pragma omp atomic
                        ...
                        debug_file << t << ",transfer," << agent->agent_uid << "," << link_id << "->" << next_link_id
                                   << "," << next_link.entrance_queue.size() << ",node_transfer\n";
                    }
                    else {
                        // Log capacity constraint
                        debug_file << t << ",blocked," << agent->agent_uid << "," << link_id << ","
                                   << next_link.entrance_queue.size() << ",capacity_constraint\n";
                        break;
                    }
                }
            }
...
#pragma omp parallel for
  ...
  public:
    ParallelQueueSimulator(int links_count, int nodes_count) : num_links(links_count), num_nodes(nodes_count) {
    ...
    ~ParallelQueueSimulator() { debug_file.close(); }
    ...
    void loadAgents(const vector<shared_ptr<CAgent_Simu>>& agent_list, int demand_starting_time_in_simu_inteval) {
        ...
        for (auto& agent : agents) {
            ...
            debug_file << "0,init," << agent->agent_uid << ",0,0,departure_time=" << agent->departure_time_in_min
                       << "\n";

            if (!agent->path_link_sequence.empty()) {
                int first_link = agent->path_link_sequence[0];
                agent->departure_time_in_simu_interval =
                    (int)(agent->departure_time_in_min * 60 / number_of_seconds_per_interval) -
                    demand_starting_time_in_simu_inteval;
...
int SimulationAPI() {
    ...
    printf("# of nodes= %d, largest zone id = %d, First Through Node (Seq No) = %d, number of links = %d\n", no_nodes,
           no_zones, FirstThruNode, number_of_links);
    ...
    int total_simu_time_intervals =
        (demand_period_ending_hours - demand_period_starting_hours) * 3600 / number_of_seconds_per_interval;
...
double getPointToLineDistance(const GDPoint* pt, const GDPoint* fromPt, const GDPoint* toPt, double unitGridRes,
                              bool no_intersection_requirement) {
    double lineLength = getEuclideanDistance(fromPt, toPt);
    if (lineLength < 1e-6) {
        return getEuclideanDistance(pt, fromPt);
    }
    double U = ((pt->x - toPt->x) * (fromPt->x - toPt->x) + (pt->y - toPt->y) * (fromPt->y - toPt->y)) /
    ...
    return no_intersection_requirement ? min({d1, d0, d2}) : d1;
}
...
struct MMLink {
    ...
    MMLink(const GDPoint& f, const GDPoint& t, const int& from_node_id, const int& to_node_id)
        : fromPt(f), toPt(t), external_from_node_id(from_node_id), external_to_node_id(to_node_id),
          link_distance(getEuclideanDistance(&f, &t)), bInsideFlag(false), likelihood_distance(1e9),
          likely_trace_no(-1), hit_count(0) {}
};
...
struct GridCell {
    ...
    vector<int> linkIndices;                // indices of MMLinks overlapping this cell
    vector<int> OriginZoneNodeIndices;      // indices of MMLinks overlapping this cell
    vector<int> DestinationZoneNodeIndices; // indices of MMLinks overlapping this cell
    vector<GPSPoint> gpsPoints;             // GPS points falling in this cell
...
public:
    ...
    void insertLink(int linkIndex, const GDPoint& fromPt, const GDPoint& toPt, int internal_from_node_id,
                    int internal_to_node_id) {
        double minX = min(fromPt.x, toPt.x);
        double maxX = max(fromPt.x, toPt.x);
        double minY = min(fromPt.y, toPt.y);
        double maxY = max(fromPt.y, toPt.y);
        ...
        for (int r = rowStart; r <= rowEnd; ++r) {
            for (int c = colStart; c <= colEnd; ++c) {
                if (isValidCell(r, c)) {
                    ...
                    if (internal_from_node_id <
                        m_FirstThruNode) { // connector, internal_from_node_id is the TAZ centroid
    ...
    void insertGPSPoint(const GPSPoint& gps, const GDPoint& pt) {
        ...
        if (isValidCell(r, c)) {
            ...
            grid_[r][c].start_min = min(grid_[r][c].start_min, timeMin);
            grid_[r][c].end_min = max(grid_[r][c].end_min, timeMin);
        }
    }
    ...
    void processCells(const vector<MMLink>& links, vector<double>& linkGeneralCost,
                      vector<int>& linkMatchingTraceNo, double nonHitDistanceRatio = 10.0) {
                ...
                if (!cell.gpsPoints.empty() && !cell.linkIndices.empty()) {
                    ...
                    for (int linkIdx : cell.linkIndices) {
                        if (linkMatchingTraceNo[linkIdx] < 0) {
                            ...
                            int bestTrace = -1;
                            for (const auto& gps : cell.gpsPoints) {
                                double d = getPointToLineDistance(&gps.pt, &links[linkIdx].fromPt, &links[linkIdx].toPt,
                                                                  1.0, false);
                                if (d < min_d) {
                                    min_d = d;
                                    bestTrace = gps.trace_no;
                                }
                            }
    ...
    void printSummary() const {
        cout << "MatchingGrid: " << rows_ << " rows x " << cols_ << " cols, resolution " << gridRes_ << "\n";
        cout << "Bounds: x[" << left_ << ", " << right_ << "], y[" << bottom_ << ", " << top_ << "]\n";
    }
...
// Outer key: agent_id (string)
// Inner key: trace_no (int)
// Value: vector of GPSPoints that belong to that trace.
int readGPSTraceFile(unordered_map<string, vector<GPSPoint>>& gpsTraces)
{
    ...
    if (parser_trace.OpenCSVFile("trace.csv", true)) {
        ...
        cerr << "Error: Cannot open trace.csv\n";
        std::exit(1);
    }
...
int determineZoneNode(const vector<int>& candidateNodeIds, const GDPoint& avgPt,
                      const map<int, GDPoint>& zoneNodeCoords) {
...
void determineOriginDestinationNodes(const vector<GPSPoint>& trace, const MatchingGrid& grid,
                                     const map<int, GDPoint>& zoneNodeCoords, int numPointsToAverage,
                                     int& originNodeId, int& destinationNodeId) {
    if (trace.empty()) {
        originNodeId = -1;
        destinationNodeId = -1;
        return;
    ...
    int countOrigin = min(numPointsToAverage, static_cast<int>(trace.size()));
    ...
    GDPoint avgDest = {0, 0};
    int countDest = min(numPointsToAverage, static_cast<int>(trace.size()));
...
int mapmatchingAPI() {
    ...
    for (const auto& L : mmLinks) {
        left = min({left, L.fromPt.x, L.toPt.x});
        right = max({right, L.fromPt.x, L.toPt.x});
        bottom = min({bottom, L.fromPt.y, L.toPt.y});
        top = max({top, L.fromPt.y, L.toPt.y});
    }
    ...
    for (size_t i = 0; i < mmLinks.size(); i++) {
        mGrid.insertLink(static_cast<int>(i), mmLinks[i].fromPt, mmLinks[i].toPt, mmLinks[i].external_from_node_id,
                         mmLinks[i].external_to_node_id);
    }
    unordered_map<string, vector<GPSPoint>> gpsTraces;
    unordered_map<string, int> gpsTraces_originNodeId;
    unordered_map<string, int> gpsTraces_destinationNodeId;
    unordered_map<string, int> gpsTraces_RouteId;
    readGPSTraceFile(gpsTraces);

    map<int, GDPoint> zoneNodeCoords;
    ...
    vector<vector<vector<int>>> ODRouteIndices(no_zones + 1,
                                                              vector<vector<int>>(no_zones + 1));
    // ------------------------
    ...
    for (const auto& agentPair : gpsTraces) {
        ...
        mGrid.cleanGPSPoint();
        const string& agentId = agentPair.first;
        ...
        // cout << "Processing agent " << agentId << ", trace " << traceNo << std::endl;
        ...
        determineOriginDestinationNodes(gpsPoints, mGrid, zoneNodeCoords, numPointsToAverage, originNodeId,
                                        destinationNodeId);
    ...
    for (const auto& agentPair : gpsTraces) {
        ...
        // ------------------------
        vector<double> linkGeneralCost(mmLinks.size(), DEFAULT_GRID_RESOLUTION);
        vector<int> linkMatchingTraceNo(mmLinks.size(), -1);
