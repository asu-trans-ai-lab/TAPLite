...
public:
    ...
    CDTACSVParser()
        : Delimiter{','}, IsFirstLineHeader{true}, m_bSkipFirstLine{false}, m_bDataHubSingleCSVFile{false},
...
bool CDTACSVParser::OpenCSVFile(std::string fileName, bool b_required) {
    ...
    if (IsFirstLineHeader) {
        ...
        if (s.size() >= 3 && (unsigned char)s[0] == UTF8_BOM[0] && (unsigned char)s[1] == UTF8_BOM[1] &&
