...
bool IsMovementRestricted(int ib_link_id, int ob_link_id) {
    if (global_movement_restrictions.find(ib_link_id) != global_movement_restrictions.end()) {
        if (global_movement_restrictions[ib_link_id].find(ob_link_id) !=
            global_movement_restrictions[ib_link_id].end()) {
...
double FindMinCostRoutes(int*** MinPathPredLink) {
    ...
    CostTo = (double**)Alloc_2D(
        no_zones, no_nodes,
        sizeof(
            double)); // cost to is based on the zone number (original node numbers) and # of nodes (node sequence no.)
...
void WriteOutputFiles(const char* filename, ODPathInfo** odPathInfoMatrix) {
    ...
    double grand_totalDistance = 0.0, grand_totalFreeFlowTravelTime = 0.0, grand_totalTravelTime = 0.0,
           grand_total_count = 0;
    ...
    for (int Orig = 0; Orig < number_of_internal_zones; ++Orig) {
        for (int Dest = 0; Dest < number_of_internal_zones; ++Dest) {
            ...
            outputFile << g_node_vector[internal_o_node_id].node_id << "," << g_node_vector[internal_d_node_id].node_id
                       << "," << pathInfo.distance << "," << (pathInfo.distance * 1.609) << "," << pathInfo.freeFlowTime
                       << "\n";
        }
    }
    ...
    if (grand_total_count > 0) {
        std::cout << "---------- Summary Statistics ----------\n"
                  ...
                  << "Average free flow travel time: " << grand_totalFreeFlowTravelTime / grand_total_count
                  << " minutes\n";
    }
...
void WriteZoneAccessibilityCSV(const char* filename, ODPathInfo** odPathInfoMatrix) {
    ...
    // destination_count, destination_avg_distance_mile, destination_avg_distance_km, destination_avg_free_flow,
    // destination_avg_congestion
    outputFile << "zone_id,origin_count,origin_avg_distance_mile,origin_avg_distance_km,origin_avg_free_flow,origin_"
                  "avg_congestion,"
               << "destination_count,destination_avg_distance_mile,destination_avg_distance_km,destination_avg_free_"
                  "flow,destination_avg_congestion\n";
    ...
    for (int zoneIdx = 0; zoneIdx < number_of_internal_zones; ++zoneIdx) {
        ...
        // Write CSV row.
        outputFile << zone_id << "," << origCount << "," << origAvgDistance << "," << origAvgDistanceKM << ","
                   << origAvgFreeFlow << "," << origAvgCongestion << "," << destCount << "," << destAvgDistance << ","
                   << destAvgDistanceKM << "," << destAvgFreeFlow << "," << destAvgCongestion << "\n";
    }
...
int ComputeAccessibilityAndODCosts_v1(const char* filename) {
    ...
    ODPathInfo** odPathInfoMatrix =
        (ODPathInfo**)Alloc_2D(number_of_internal_zones, number_of_internal_zones, sizeof(ODPathInfo));
...
int ComputeAccessibilityAndODCosts_v2(const char* filename) {
    ...
    for (int batchStart = 0; batchStart < number_of_internal_zones; batchStart += batch_size) {
        ...
        ODPathInfo** batchMatrix =
            (ODPathInfo**)Alloc_2D(currentBatchSize, number_of_internal_zones, sizeof(ODPathInfo));
        ...
        for (int localOrigin = 0; localOrigin < currentBatchSize; ++localOrigin) {
            ...
            for (int Dest = 0; Dest < number_of_internal_zones; ++Dest) {
                ...
                outputFile << g_node_vector[internal_o_node_id].node_id << ","
                           << g_node_vector[internal_d_node_id].node_id << "," << info.distance << ","
                           << (info.distance * 1.609) << "," << info.freeFlowTime << "\n";
            }
    ...
    zoneFile << "zone_id,origin_count,origin_avg_distance_mile,origin_avg_distance_km,origin_avg_free_flow,origin_avg_"
                "congestion,"
             << "destination_count,destination_avg_distance_mile,destination_avg_distance_km,destination_avg_free_flow,"
                "destination_avg_congestion\n";
    ...
    for (int zoneIdx = 0; zoneIdx < number_of_internal_zones; zoneIdx++) {
        ...
        zoneFile << zone_id << "," << origCount << "," << origAvgDistance << "," << origAvgDistanceKM << ","
                 << origAvgFreeFlow << "," << origAvgCongestion << "," << destCount << "," << destAvgDistance << ","
                 << destAvgDistanceKM << "," << destAvgFreeFlow << "," << destAvgCongestion << "\n";
    }
    zoneFile.close();
...
#pragma omp critical
                        ...
                        if (linkIndices.size() > 0) {
                            if (route_output_flag || Assignment_iteration_no == 0) {
                                ...
                                AddLinkSequence(m, origin_seq_no, destination_seq_no, Assignment_iteration_no,
                                                currentLinkSequence);
...
void performODME(std::vector<double> theta, double* MainVolume, struct link_record* Link) {
    ...
    for (int iter = 0; iter < maxIter; ++iter) {
        ...
        cout << "iter = " << iter << " [Deviation Log] OD avg deviation = " << avgODDev
             << ", Link avg deviation = " << avgLinkDev << ", VMT deviation = " << vmtDev << " ";
            ...
            for (int Orig = 1; Orig <= numZones; ++Orig) {
                for (int Dest = 1; Dest <= numZones; ++Dest) {
                    ...
                    for (auto& entry : linkUsage) {
                        ...
                        if (Link[link_id].Obs_volume[1] > 1) {
                            grad_link +=
                                2.0 * w_link * (linkFlows[link_id] - Link[link_id].Obs_volume[1]) * sensitivity;
                        }
                        if (VMT_target > 1) {
        ...
        while (true) {
                ...
                for (int Orig = 1; Orig <= numZones; ++Orig)
                    for (int Dest = 1; Dest <= numZones; ++Dest) {
                        ...
                        if (old_MDODflow[m][Orig][Dest] > 0.001) {
                            candidate_MDODflow[m][Orig][Dest] =
                                old_MDODflow[m][Orig][Dest] - alpha * gradient_MDODflow[m][Orig][Dest];
                            ...
                            if (candidate_MDODflow[m][Orig][Dest] < lesser_value * 0.5)
                                candidate_MDODflow[m][Orig][Dest] =
                                    lesser_value * 0.5; // ensure non-negativity and resonableness

                            if (candidate_MDODflow[m][Orig][Dest] > larger_value * 1.5)
                                candidate_MDODflow[m][Orig][Dest] =
                                    larger_value * 1.5; // ensure uppper bound resonableness
                        }
                    }

            double F_candidate = computeObjectiveForMDOD(candidate_MDODflow, r2);
            logFile << "[Line Search] alpha = " << alpha << " | F_candidate = " << F_candidate
                    << " | F_current = " << F_current << " | Threshold = " << (F_current - armijo_c * alpha * sumGradSq)
                    << "\n";
            ...
            logFile << "Iteration " << iter << ": Step size chosen = " << alpha << ", Objective: " << F_current
                    << " -> " << computeObjectiveForMDOD(candidate_MDODflow, r2) << "\n";
            cout << "Iteration " << iter << ": Step size chosen = " << alpha << ", Objective: " << F_current << " -> "
                 << computeObjectiveForMDOD(candidate_MDODflow, r2) << "\n";
    ...
    for (int m = 1; m <= numModes; ++m) {
        ...
        double meanObs = (validCount > 0) ? (sumObs / validCount) : 0.0;
        logFile << "Total valid links: " << validCount << ", Sum of observed volumes: " << sumObs
        ...
        for (size_t l = 1; l < linkFlows.size(); ++l) {
            if (Link[l].Obs_volume[1] > 1) {
                ...
                logFile << "Link " << l << ": Predicted volume: " << pred << ", Observed volume: " << obs
                        << ", Difference: " << diff << ", Squared diff: " << squaredDiff
        ...
        logFile << "Computed Pearson correlation: " << r_corr << ", R2 (from correlation): " << r2_correlation << "\n";
...
void OutputRouteDetails(const std::string& filename, std::vector<double> theta) {
    ...
    outputFile_vehicle << "agent_id,departure_time,departure_time_hhmmss,mode,route_id,o_zone_id,d_zone_id,unique_"
                ...
                for (const auto& pair : uniqueRoutes) {
                    ...
                    if (no_zones < 3000 || (no_zones >= 3000 && od_volume >= 1.0)) {
                        ...
                        outputFile << g_mode_type_vector[m].mode_type.c_str() << "," << rd.firstRouteID
                                   ...
                                   << ","
                                   //<< rd.linkLengthStr << ","
                                   << rd.totalDistance << "," << rd.totalDistance * 1.609 << ","
                                   << rd.totalFreeFlowTravelTime << "," << rd.totalTravelTime << "," << rd.routeKey
                                   << "," << seed_od_volume << "," << target_od_volume << "," << od_volume << ","
                        ...
                        if (vehicle_log_flag == 1 && !linkIndices[m][Orig][Dest][rd.firstRouteID].empty()) {
                            ...
                            int total_time_window_min =
                                (demand_period_ending_hours - demand_period_starting_hours) * 60;
                            ...
                            if (integer_volume > 0) {
                                ...
                                for (int v = 0; v < integer_volume; ++v) {
                                    ...
                                    outputFile_vehicle
                                        << agent_id << "," << departure_time << "," << departure_time_hhmmss << ","
                                        << g_mode_type_vector[m].mode_type.c_str() << "," << rd.firstRouteID << ","
                                        << org_origin_zone << "," << org_dest_zone << "," << unique_route_id << ","
...
void OutputODPerformance(const std::string& filename) {
    ...
    googleMapsFile << "route_key,mode,o_zone_id,d_zone_id,volume,total_distance_mile,total_distance_km,straight_line_"
                      "distance_mile,straight_line_distance_km,distance_ratio,total_free_flow_travel_time,total_"
                      "congestion_travel_time,google_maps_http_link,WKT_geometry\n";

    outputFile << "route_key,mode,o_zone_id,d_zone_id,o_x_coord,o_y_coord,d_x_coord,d_y_coord,total_distance_mile,"
                  "total_distance_km,straight_line_distance_mile,straight_line_distance_km,distance_ratio,total_free_"
                  "flow_travel_time,total_congestion_travel_time,volume,WKT_geometry\n";
            ...
            for (int Dest = 1; Dest < linkIndices[m][Orig].size(); ++Dest) {
                ...
                for (int route_id = 0; route_id < linkIndices[m][Orig][Dest].size(); ++route_id) {
                    if (!linkIndices[m][Orig][Dest][route_id].empty()) {
                        ...
                        std::string routeKey =
                            std::to_string(Orig) + "_" + std::to_string(Dest) + "_" + g_mode_type_vector[m].mode_type;
                        ...
                        if (uniqueRoutes.find(routeKey) == uniqueRoutes.end()) {
                            ...
                            double straight_line_distance_mile =
                                3958.8 * 2 *
                                atan2(sqrt(pow(sin((d_y_coord - o_y_coord) * M_PI / 360.0), 2) +
                                           cos(o_y_coord * M_PI / 180.0) * cos(d_y_coord * M_PI / 180.0) *
                                               pow(sin((d_x_coord - o_x_coord) * M_PI / 360.0), 2)),
                                      sqrt(1 - pow(sin((d_y_coord - o_y_coord) * M_PI / 360.0), 2) -
                                           cos(o_y_coord * M_PI / 180.0) * cos(d_y_coord * M_PI / 180.0) *
                                               pow(sin((d_x_coord - o_x_coord) * M_PI / 360.0), 2)));
                            double straight_line_distance_km = straight_line_distance_mile * 1.609;

                            double distance_ratio = (straight_line_distance_mile > 0.001)
                                                        ? totalDistance / straight_line_distance_mile
                                                        : 1.0;

                            grand_total_straight_line_distance += straight_line_distance_mile * volume;
                            grand_total_distance_ratio += distance_ratio * volume;

                            outputFile << routeKey << "," << g_mode_type_vector[m].mode_type << "," << Orig << ","
                                       << Dest << "," << o_x_coord << "," << o_y_coord << "," << d_x_coord << ","
                                       << d_y_coord << "," << totalDistance << "," << totalDistance * 1.609 << ","
                                       << straight_line_distance_mile << "," << straight_line_distance_km << ","
                                       << distance_ratio << "," << totalFreeFlowTravelTime << "," << totalTravelTime
                                       << "," << volume << ",\"" << WKT_geometry << "\"\n";

                            if (volume >= 1.0 && no_zones <= 3000 || volume >= 10.0 && no_zones > 3000) {
                                std::string googleMapsLink =
                                    "https://www.google.com/maps/dir/" + std::to_string(o_y_coord) + "," +
                                    std::to_string(o_x_coord) + "/" + std::to_string(d_y_coord) + "," +
                                    std::to_string(d_x_coord) + "/";

                                googleMapsFile << routeKey << "," << g_mode_type_vector[m].mode_type << "," << Orig
                                               << "," << Dest << "," << volume << "," << totalDistance << ","
                                               << totalDistance * 1.609 << "," << straight_line_distance_mile << ","
                                               << straight_line_distance_km << "," << distance_ratio << ","
                                               << totalFreeFlowTravelTime << "," << totalTravelTime << ",\""
                                               << googleMapsLink << "\",\"" << WKT_geometry << "\"\n";
                            }
                        }
                    }
                }
...
void GenerateAggregatedPerformanceAndAccessibility() {
    ...
    for (const auto& kv : modeAgg) {
        ...
        systemPerfFile << mode << "," << data.total_volume << "," << data.total_VMT << "," << data.total_VHT << ","
                       << data.total_delay << "," << TTI << "," << avg_distance_mile << "," << avg_distance_km << ","
                       << avg_travel_time_min << "," << avg_travel_delay_min << "," << avg_speed_mph << ","
                       << avg_speed_kmph << "\n";
    }
    ...
    originFile << "origin_zone_id,total_volume,number_of_destinations,avg_distance_mile,avg_travel_time_min,google_"
                  "maps_link\n";
    for (int o = 1; o <= no_zones; ++o) {
        ...
        if (g_map_external_node_id_2_node_seq_no[o] < g_node_vector.size()) {
            ...
            googleLink =
                "https://www.google.com/maps/search/?api=1&query=" + std::to_string(y) + "," + std::to_string(x);
        }
        // Enclose the google link in double quotes.
        originFile << o << "," << vol << "," << originStats[o].count_dest << "," << avg_distance << ","
                   << avg_travel_time << ","
                   << "\"" << googleLink << "\""
                   << "\n";
    }
    ...
    destFile << "destination_zone_id,total_volume,number_of_origins,avg_distance_mile,avg_travel_time_min,google_maps_"
                "link\n";
    for (int d = 1; d <= no_zones; ++d) {
        ...
        destFile << d << "," << vol << "," << destinationStats[d].count_orig << "," << avg_distance << ","
                 << avg_travel_time << ","
                 << "\"" << googleLink << "\""
                 << "\n";
    }
            ...
            for (int Dest = 1; Dest < linkIndices[m][Orig].size(); ++Dest) {
                ...
                if (!routeExists && MDODflow[m][Orig][Dest] > 0) {
                    ...
                    if (Orig < g_map_external_node_id_2_node_seq_no.size() &&
                        ...
                        googleLink = "https://www.google.com/maps/dir/" + std::to_string(o_y) + "," +
                                     std::to_string(o_x) + "/" + std::to_string(d_y) + "," + std::to_string(d_x) + "/";
                    }

                    int org_origin_zone = g_map_node_seq_no_2_external_node_id[Orig];
                    int org_dest_zone = g_map_node_seq_no_2_external_node_id[Dest];

                    inacFile << modeStr << "," << org_origin_zone << "," << org_dest_zone << ","
                             << "\"" << googleLink << "\""
                             << "\n";
                }
            }
    ...
    if (parser_node.OpenCSVFile("node.csv", true)) {
                                         ...
                                         // field headers.
        {
            ...
            if (zone_id >= 1) {
                ...
                g_map_internal_zone_no_2_node_seq_no[g_node_vector[internal_node_id].internal_zone_no] =
                    internal_node_id;
...
void createSettingsFile(const std::string& fileName) {
    ...
    file << "number_of_iterations,number_of_processors,demand_period_starting_hours,demand_period_ending_hours,first_"
            "through_node_id,base_demand_mode,route_output,vehicle_output,simulation_output,log_file,odme_mode,odme_"
            "vmt\n";
...
void InitializeLinkIndices(int num_modes, int no_sequential_zones, int max_routes) {
    ...
    linkIndices = std::vector<std::vector<std::vector<std::vector<std::vector<int>>>>>(
        ...
        std::vector<std::vector<std::vector<std::vector<int>>>>(
            no_sequential_zones + 1, std::vector<std::vector<std::vector<int>>>(
                                         no_sequential_zones + 1, std::vector<std::vector<int>>(max_routes + 1))));
...
int AssignmentAPI() {
    ...
    if (first_through_node_id_input >= 1) {
        if (g_map_external_node_id_2_node_seq_no.find(first_through_node_id_input) !=
            ...
            FirstThruNode =
                g_map_external_node_id_2_node_seq_no[first_through_node_id_input]; // equal to external input from
                                                                                   // seetings.csv
        }
        else {
    ...
    printf("# of nodes= %d, largest zone id (# of zones) = %d, First Through Node ID = %d, number of links = %d\n",
           no_nodes, no_zones, g_node_vector[FirstThruNode].node_id, number_of_links);
    ...
    fprintf(link_performance_file,
            "iteration_no,link_id,vdf_type,from_node_id,to_node_id,length_meter,vdf_length_mi,vehicle_volume,person_"
            "volume,ref_vhc_olume,base_demand_volume,obs_volume,background_volume,"
            "link_capacity,lane_capacity,D,doc,vdf_fftt,travel_time,vdf_distance_factor,vdf_alpha,vdf_beta,vdf_plf,"
            "speed_mph,speed_kmph,VMT,VHT,PMT,PHT,VHT_QVDF,PHT_QVDF,geometry,");
    ...
    fprintf(link_performance_file,
            ...
            "queue_speed_kmph,avg_QVDF_period_speed_kmph,avg_QVDF_period_travel_time,Severe_Congestion_P,");
...
int Read_ODtable(double*** ODtable, double*** DiffODtable, double*** Seed_ODtable, double*** target_ODtable,
                 int no_zones) {
...
inline double travel_time_ratio(double x, double alpha, double beta) {
    ...
    return 2.0 + std::sqrt(alpha * alpha * one_minus_x * one_minus_x + beta * beta) - alpha * one_minus_x - beta;
}
...
double Link_Travel_Time_Integral_Conical(int k, double* Volume) {
    ...
    // primitive at x
    double Ix = 2.0 * x - 0.5 * u * S - (β * β) / (2 * α) * std::log(α * u + S) - α * (x - 0.5 * x * x) - β * x;

    // primitive at 0 (u=1, x=0)
    double u0 = 1.0;
    double S0 = std::sqrt(α * α * u0 * u0 + β * β);
    double I0 = 0.0                                                               // 2·0
                - 0.5 * u0 * S0 - (β * β) / (2 * α) * std::log(α * u0 + S0) - 0.0 // α·(0−0²/2)
                - 0.0;                                                            // β·0
...
public:
    ...
    void logQueueStatus(int t, int link_id, int entrance_size, int exit_size) {
        if (LOG_DEBUG <= current_level) {
            debug_file << t << ",DEBUG,QUEUE," << link_id << "," << entrance_size << "," << exit_size << endl;
        }
    }

    void logSimulationStatus(int t, int loaded, int transfers, int completed) {
        if (LOG_INFO <= current_level) {
            info_file << t << ",INFO,STATUS," << loaded << "," << transfers << "," << completed << endl;
        }
    }
...
public:
    ...
    void logAgent(const shared_ptr<CAgent_Simu>& agent, int l_demand_period_starting_hours) {
        ...
        agent_file << agent->agent_uid << "," << agent->departure_time_in_min << ","
                   ...
                   << agent->o_zone_id << "," << agent->d_zone_id << "," << agent->distannce << "," << travel_time
...
public:
    ...
    void processCells(const std::vector<MMLink>& links, std::vector<double>& linkGeneralCost,
                      std::vector<int>& linkMatchingTraceNo, double nonHitDistanceRatio = 10.0) {
    ...
    void printSummary() const {
        std::cout << "MatchingGrid: " << rows_ << " rows x " << cols_ << " cols, resolution " << gridRes_ << "\n";
...
void determineOriginDestinationNodes(const std::vector<GPSPoint>& trace, const MatchingGrid& grid,
                                     const std::map<int, GDPoint>& zoneNodeCoords, int numPointsToAverage,
...
void mapmatchingAPI() {
    ...
    std::vector<std::vector<std::vector<int>>> ODRouteIndices(no_zones + 1,
...
void DTA_AssignmentAPI() { AssignmentAPI(); }

void DTA_SimulationAPI() { SimulationAPI(); }
