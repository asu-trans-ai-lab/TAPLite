...
void log_accessibility(int*** MinPathPredLink)
{
	double** CostTo;
	...
	FILE* f = fopen("zone_access_map.csv", "w");
	if (!f) {
		perror("fopen accessibility CSV");
		return;
	}
	
	fprintf(f, "mode_id,zone_id,node_id,x_coord,y,dist_label\n");
			...
			for (int m = 1; m <= number_of_modes; m++)
			{
				...
				if (MDRouteCost != NULL)
				{
					...
					for (int n = 1; n <= no_nodes; ++n) {
						double d = CostTo[Orig][n];
						int k = MinPathPredLink[m][Orig][n];
						
						if (d < BIGM) {
							fprintf(f, "%d,%d,%d,%.6f,%.6f,%.3f\n",
								m,Orig, g_node_vector[n].node_id, g_node_vector[n].x, g_node_vector[n].y, d);
						}
					}
				}
			}
...
int AssignmentAPI()
{
	...
	fprintf(logfile, "iteration_no,link_no,link_id,from_node_id,to_node_id,volume,ref_volume,obs_volume,capacity,doc,fftt,travel_time,delay,");
	...
	log_accessibility(MDMinPathPredLink); // for debugging 
	double gap;
	...
	if (g_tap_log_file == 1)
	{
		for (int k = 1; k <= number_of_links; k++)
		{
			fprintf(logfile, "%d,%d,%d,%d,%d,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,",
				iteration_no, k, Link[k].link_id, Link[k].external_from_node_id, Link[k].external_to_node_id,
	...
	for (int k = 1; k <= number_of_links; k++)
	{
		...
		fprintf(link_performance_file, "%d,%d,%d,%d,%d,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.4lf,%.8lf,%.8lf,%.8lf,%.8lf,%.8lf,%.8lf,%.8lf,%.8lf,%.8lf,%.8lf,",
			...
			Link[k].Travel_time, Link[k].VDF_distance_factor, Link[k].VDF_Alpha, Link[k].VDF_Beta, Link[k].VDF_plf, Link[k].length / fmax(Link[k].Travel_time / 60.0, 0.00001), Link[k].length / fmax(Link[k].Travel_time / 60.0, 0.00001) * 1.609, Link[k].Travel_time - Link[k].FreeTravelTime);
...
double Link_Travel_Time_BPR(int k, double* Volume)
{
	...
	Link[k].Travel_time =
		Link[k].FreeTravelTime * (1.0 + Link[k].VDF_Alpha * (pow(IncomingDemand / fmax(0.1, Link[k].Lane_Capacity), Link[k].VDF_Beta)));
...
double Link_Conical_Travel_Time(int k, double* Volume)
{
	...
	double cap = fmax(0.1, Link[k].Lane_Capacity);
...
double Link_Travel_Time_Integral_BPR(int k, double* Volume)
{
	...
	if (Link[k].VDF_Beta >= 0.0)
		integral += (Volume[k] * Link[k].FreeTravelTime *
			(1.0 + (Link[k].BoverC / (Link[k].VDF_Beta + 1)) * pow(IncomingDemand, Link[k].VDF_Beta + 1)));

	return integral;
}

double Link_Travel_Time_Integral_Conical_version2(int k, double* Volume)
{
	double t0 = Link[k].FreeTravelTime;                          // t₀
	double c = std::fmax(0.1, Link[k].Lane_Capacity);           // capacity
...
double Link_Travel_Time_Integral_Conical(int k, double* Volume)
{
	...
	// Extract parameters
	double t0 = Link[k].FreeTravelTime;
	double c = std::fmax(0.1, Link[k].Lane_Capacity);  // Avoid divide-by-zero
	double lanes = std::fmax(0.01, Link[k].lanes);
	double period = std::fmax(0.001, demand_period_ending_hours - demand_period_starting_hours);
	double plf = std::fmax(0.0001, Link[k].VDF_plf);
	double alpha = Link[k].VDF_Alpha;

	// Guard: prevent singularity at alpha = 1
	if (std::fabs(2 * alpha - 2.0) < EPS) {
		std::cerr << "Warning: Alpha too close to 1; singularity in beta.\n";
		return 0.0;
	}

	double beta = (2 * alpha - 1) / (2 * alpha - 2);  // Spiess formulation

	// Compute normalized flow x = V / C
	double flow_rate = Volume[k] / lanes / period / plf;
	double x = flow_rate / c;

	// Guard: if x ≈ 0, integral = 0
	if (x < EPS)
		return 0.0;

	// Define helper function for primitive (anti-derivative)
	auto primitive = [alpha, beta](double x_val) -> double {
		double u = 1.0 - x_val;
		double S = std::sqrt(alpha * alpha * u * u + beta * beta);
		double term_log = std::log(alpha * u + S);
		return 2.0 * x_val
			- 0.5 * u * S
			- (beta * beta) / (2.0 * alpha) * term_log
			- alpha * (x_val - 0.5 * x_val * x_val)
			- beta * x_val;
		};

	// Compute definite integral
	double integral = primitive(x) - primitive(0.0);

	return t0 * c * integral;
}
